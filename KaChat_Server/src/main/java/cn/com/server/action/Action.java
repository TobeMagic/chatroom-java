...
public class Action {
    ...
    public void registerNotice( Object data, Socket socket,ObjectOutputStream oos) throws IOException {
       User user = (User) data;
       if (dao.containsUID(user.getID())){ // 存在则返回空
           return;
       }
       String uID = String.valueOf(100001+dao.userCount());   //没存在则设置user id 以 100001 开始计数，为string类型
       user.setID(uID);
        dao.addUser(user); // 添加到存储对象空间
        System.out.println("[register]\tIP:"+socket.getInetAddress()+",端口:"+socket.getPort()+",成功注册UID: "+uID+ ServerUtil.getDateInNow());
